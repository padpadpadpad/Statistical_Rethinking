...
Working through a binomial, logistic regression based on prosociality of chimpanzees.
...
The experiment looks at which lever (right or left) a chimpanzee pulls. One of the levers returns food items to both sides of the table whereas one of the levers returns food to only one side. We are interested in investigating whether having a chimpanzee on the other side of the table increases the probability that a chimpanzee pulls the lever that releases two food items.

This gives the simplest model of an intercept that can predict lever pulls (specifically whether the left lever was pulled) based on only `prosoc_left`, whether or not the left hand lever was (1) or was not (0) attached to the prosocial option, the side with two pieces of food. 

Simplest model:

$$L_{i} ~ Binomial(1, p_{i})$$
$$logit(p_{i} = \alpha)$$
$$\alpha ~ Normal(0,10$$

Firstly we will fit this most simple model in stan using map2stan().

```{r, message = FALSE, results='hide'}
# load in chimpanzee data
data("chimpanzees")
...
m10.1 <- map2stan(m10.1, data = chimpanzees, iter = 1e4, warmup = 1000)
...
We can fit the equivalent model purely in stan.
...
# set up data list
stan_datalist <- list(N = nrow(d), pulled_left = d$pulled_left)

# set up start values
start <- function() list(a = 1, sigma = 1)
